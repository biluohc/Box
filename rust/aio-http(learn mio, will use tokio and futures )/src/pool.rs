...
// 还要基于rayor 写个依靠函数调用, 线程复用,通过channel通信的公共阻塞io库,
//
// Unix listen envent-loop+ cpu-nums worker?
// windows iocp单进程只支持一个event-loop, 所以考虑windows只开一个worker,并且for listen同步分发(单独写一个模块/文件).
//
// 真正的文件async有下面这两个
// https://github.com/asomers/tokio-file
// https://github.com/Rufflewind/tokio-file-unix/
#![allow(deprecated)] //他根本没把mio-more放到crates.io
use mio::channel::{channel, Sender, SendError};
...
impl Pool {
    pub fn new() -> io::Result<Self> {
        ...
        for idx in 0..cpus {
            ...
            let join_handle = Builder::new().spawn(
                move || if let Err(e) = Server::new(sc, idx)
                    .event_loop()
                {
