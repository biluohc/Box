...
pub struct Client {
    ...
    pub reader: StaticMut<Vec<u8>>,
    pub writer: StaticMut<Vec<u8>>,
...
impl Client {
    ...
    pub fn keep_alive(&mut self) -> io::Result<()> {
        ...
        self.reader = StaticMut::new(Vec::new());
        self.writer = StaticMut::new(Vec::new());
        self.writer_bytes = 0;
        self.done = false;
        Ok(())
    }
    pub fn new(socket: TcpStream, live: Arc<()>) -> Self {
        Client {
            socket: socket,
            reader: StaticMut::new(Vec::new()),
            writer: StaticMut::new(Vec::new()),
            ...
            match self.socket.read(&mut buf) {
                Ok(size) => {
                    if size == 0 {
                        ...
                        self.reader.as_mut().extend_from_slice(&buf[..size]);
                    }
                }
                Err(err) => {
    ...
    pub fn write(&mut self) -> io::Result<()> {
        // let mut data = self.writer.as_mut();
